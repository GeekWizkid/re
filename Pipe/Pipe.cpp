...
void ExecuteVsCommand(IDispatch* pVsDte, const wchar_t* commandName)
{
	if(!pVsDte || !commandName)
		return;

	VARIANT commandVariant;
	VariantInit(&commandVariant);
	commandVariant.vt = VT_BSTR;
	commandVariant.bstrVal = SysAllocString(commandName);

	VARIANT argsVariant;
	VariantInit(&argsVariant);
	argsVariant.vt = VT_BSTR;
	argsVariant.bstrVal = SysAllocString(L"");

	// AutoWrap forwards args as-is, while IDispatch expects reversed rgvarg order.
	AutoWrap(DISPATCH_METHOD, NULL, pVsDte, L"ExecuteCommand", 2, argsVariant, commandVariant);
...
{	
	...
	HRESULT hr = pGIT->GetInterfaceFromGlobal(dwDebuggerCookie, IID_IDispatch, (void **)&pDebugger);
	if(FAILED(hr) || !pDebugger)
		return 0;

	IDispatch *pVsDte = NULL;
	if(dwDteCookie)
		pGIT->GetInterfaceFromGlobal(dwDteCookie, IID_IDispatch, (void **)&pVsDte);
	...
	while(WaitForSingleObject(hStopEvent, 0) != WAIT_OBJECT_0) {
		...
		for(std::vector<SOCKET>::iterator it = gClients.begin(); it != gClients.end();) {
			...
			if (!hEdit) goto EndFindWindow;
				...
				Sleep(80);
				ExecuteVsCommand(pVsDte, L"Debug.GoToSourceCode");
			}
			EndFindWindow:
			++it;
		}
...
extern "C" PIPE_API void _Connect(HWND hWnd, IDispatch* _pDTE, IDispatch* _pDebugger)
{
	...
	if(FAILED(hr) || !pGIT)
		return;
	pGIT->RegisterInterfaceInGlobal(_pDebugger, IID_IDispatch, &dwDebuggerCookie);
	pGIT->RegisterInterfaceInGlobal(_pDTE, IID_IDispatch, &dwDteCookie);
...
extern "C" PIPE_API void _Disconnect()
{
	...
	if(pGIT) {
		if(dwDebuggerCookie)
			pGIT->RevokeInterfaceFromGlobal(dwDebuggerCookie);
		if(dwDteCookie)
			pGIT->RevokeInterfaceFromGlobal(dwDteCookie);
		pGIT->Release();
		pGIT = NULL;
		dwDebuggerCookie = 0;
		dwDteCookie = 0;
